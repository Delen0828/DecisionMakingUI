...
A static decision-making interface presented as a humanitarian aid deployment portal. Select a location on a Leaflet map to view its assessment, deployment method, related posts, and urgency/trust distributions. The interface uses local JSON files and requires no build step or backend.
...
These settings are edited directly in the CSS or JavaScript in `index.html`:

| Setting | Location / default |
| --- | --- |
| Base palette | CSS `:root`: background `#f5f5f5`, text `#000000`, primary/header `#0072CE`, high `#cc0000`, mid `#ff9900`, low `#006600`. Some component colors are hardcoded separately. |
| Initial map center and zoom | `initializeMainMap()`: latitude/longitude `[5.1521, 46.1996]`, zoom `6`. |
| Map tiles and maximum zoom | `L.tileLayer()` in `initializeMainMap()`: OpenStreetMap tiles, `maxZoom: 18`. |
| Map controls | `initializeMainMap()`: `zoomControl: true`, `attributionControl: false`. |
| Marker/coverage colors | `addAllMarkersToMap()`: incident urgency `high` is red, `mid` is orange, other values are green. |
| Coverage radius | Each incident's `radius_m`, in meters. |
| Related-post ordering | `displayTarget()`: urgency high → medium → low, then trust high → medium → low, then newest timestamp first. |
| Data file paths | The three `fetch()` calls load `incident.json`, `posts_expanded.json`, and `source.json`. |

## Data files

| File | Purpose / fields |
| --- | --- |
| [incident.json](incident.json) | Location records: `Target ID`, `Target Name`, `Target Category`, `Cluster Damage`, `Certainty`, `Suggested Strike Method`, `AI Description`, `latitude`, `longitude`, `radius_m`, and `source_ids`. Legacy field names are retained; the UI labels them as aid locations, urgency, and deployment methods. Incident urgency uses `low`, `mid`, or `high`. |
...
| [posts.json](posts.json) | Alternate post dataset; not loaded by the current page. |
...
| --- | --- |
| API key | Required `OPENAI_API_KEY` environment variable; never commit a real key. |
...
Install the `openai` Python package in a virtual environment and set `OPENAI_API_KEY` in your environment before running `python3 generate_descriptions.py`. Input records must include `Target Name`, `Target Category`, `Cluster Damage`, `Certainty`, `Suggested Strike Method`, and `Source`. The script adds an `AI Description` field. The current incident dataset has no `Source` field, so it cannot be used unchanged. Generated output is not automatically loaded by the UI.

## Other files

`rename.sh` renames PNG files in `img/` to sequential numbers starting at `1`. That directory is not included. The script changes filenames in place and can overwrite existing numbered files; review the directory before using it.
